...
class Hangman():
    ...
    def play(self):
        ...
        """
        while True:
            self.print_game_status() # 현재 게임 상태를 먼저 출력해줌.
            user_input = self.get_user_input() # user_input 을 입력받는 메소드 적용.

            # Validate the user input
            if self.is_invalid_letter(user_input): # 플레이어가 입력한 인풋이 유효한 값인지 아닌지.
                print('reenter')# 아니면, 제대로 입력하라는 메세지 출력하고
                continue # break or continue 중 하나를 사용하여 계속 입력 받음.
            
            # Check if the letter is not already guessed
            if self.user_input in self.find_indexes(self.user_input):
                print('already input') # 이미 입력받았던 알파벳이면, 이미 입력했다 메시지 출력
                continue  # break or continue 중 하나를 사용하여 계속 입력

            # when the user_input is a character in word_to_guess
            if self.user_input in self.word_to_guess:
                indexes = self.find_indexes() # 추측한 글자의 인덱스 받기
                self.update_progress() # 입력받은 글자로 현재 게임 상태 업데이트

                # If there is no letter to find in the word
                if indexes == self.word_to_guess: # 더이상 맞출 글자가 없으면
                    print('\n------Yay! You win!')
